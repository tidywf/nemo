...
tidy name (snake_case), type, description, and the `since` version it was introduced.
...
The `since` field on each column controls which schema version it belongs to.
`latest` means the column is present in the most recent version; explicit tags
(e.g. `v1.2.3`) mark columns introduced in that version.
