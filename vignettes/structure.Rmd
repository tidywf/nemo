...
Below we can see that the `input_id` specified is added to the written files in
an additional `input_id` column. This can be used e.g. to distinguish results
from different runs in a data pipeline. An optional `output_id` column can also
be added, and an `input_pfix` column can be included via `pfix_include = TRUE`.
