...
`get_schemas_all()` derives a versioned view from the unified column list.
Pass `"raw"` (default) or `"tidy"` to get column names in either form:

```{r tool1_config_schemas}
(s_raw <- conf$get_schemas_all("raw"))
s_raw |>
  dplyr::filter(.data$name == "table1", .data$version == "v1.2.3") |>
  dplyr::select("schema") |>
  tidyr::unnest("schema")

(s_tidy <- conf$get_schemas_all("tidy"))
s_tidy |>
  dplyr::filter(.data$name == "table1", .data$version == "v1.2.3") |>
  dplyr::select("schema") |>
  tidyr::unnest("schema")
...
t1$get_raw_schema("table1", v = "v1.2.3")
# t1$get_raw_schema("table1", v = "latest") # default
t1$get_tidy_schema("table1", v = "v1.2.3")
# t1$get_tidy_schema("table1", v = "latest") # default
```
...
`input_pfix` column can be included via `pfix_include = TRUE`, which would
...
# readr::read_csv(wfiles$path[1], show_col_types = F) # see tidywf/tidywigits#137
```
...
wfiles2 |>
  dplyr::select("bname", "size", "type")
# no output_id or input_pfix specified
