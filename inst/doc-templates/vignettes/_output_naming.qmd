...
identical file names but live under different parent folders. Here we show:
...
- how to carry a run identifier into the data so the runs are still
  distinguishable once the files have been merged into a merged parquet dataset
  or database.
...
`output_id = "<your id>"` with e.g. an auto-generated [ULID][ulid] (done
automatically with the CLI's `--ulid` flag via the [ulid][ulid-r] R package),
...
[ulid]: https://wiki.tcl-lang.org/page/ULID "What is ULID"
[ulid-r]: https://github.com/eddelbuettel/ulid "ULID in R"
